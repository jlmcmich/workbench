...
const make = Effect.gen(function* () {
  ...
  const processTurnQueueRequested = Effect.fn("processTurnQueueRequested")(function* (
    event: Extract<ProviderIntentEvent, { type: "thread.turn-queue-requested" }>,
  ) {
    const thread = yield* resolveThread(event.payload.threadId);
    if (!thread) {
      return;
    ...
    if (!hasSession) {
      return yield* appendProviderFailureActivity({
        threadId: event.payload.threadId,
        kind: "provider.turn.queue.failed",
        summary: "Provider queue message failed",
        detail: "No active provider session is bound to this thread.",
        turnId: null,
        createdAt: event.payload.createdAt,
      });
  ...
  const processDomainEvent = Effect.fn("processDomainEvent")(function* (
    ...
    switch (event.type) {
      ...
      case "thread.turn-interrupt-requested":
        ...
        return;
      case "thread.turn-queue-requested":
        yield* processTurnQueueRequested(event);
        return;
  ...
  const start: ProviderCommandReactorShape["start"] = Effect.fn("start")(function* () {
    const processEvent = Effect.fn("processEvent")(function* (event: OrchestrationEvent) {
      if (
        ...
        event.type === "thread.turn-queue-requested" ||
